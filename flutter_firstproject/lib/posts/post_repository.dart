  ...
  Future<bool> createPost(String userId) async {
    ...
    try {
      ...
      Post post = Post(
          ...
          // NOTE: how necessary is this? if errors occur check this one line:
          //user: user[0],
          //user model (this was already commented out)
 ...
 */
  Future<User> retrieveUser(String userId) async {
    User user = (await Amplify.DataStore.query(User.classType,
        where: User.ID.eq(userId)))[0];
    return user;
  }

  Future<Null> cropPostImage(
      String imageFilePath, BuildContext context, String targetPath) async {
    loading = true;
    var uuid = Uuid().v1();
     ...
     */
      ...
      try {
        UploadFileResult result = await Amplify.Storage.uploadFile(
            key: uuid, local: croppedFile, options: options);
